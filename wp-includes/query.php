...
class WP_Query {
	...
	function &get_posts() {
		...
		if ( empty($q['orderby']) ) {
			...
			$allowed_keys = array('name', 'author', 'date', 'title', 'modified', 'menu_order', 'parent', 'ID', 'rand', 'comment_count', 'status');
			if ( !empty($q['meta_key']) ) {
		...
		// echo '<br/>' . __FILE__ . "\r\n" . __LINE__ . " SELECT $found_rows $distinct $fields FROM $wpdb->posts $join WHERE 1=1 $where $groupby $orderby $limits " . '<br/>';
		...
		if ( !empty($this->posts) && ($this->is_single || $this->is_page) ) {
			...
			//added this line - balaji
			if ( 'trash' == $status || 'expired' == $status || 'approved' == $status || 'pending' == $status ) {$post_status_obj->protected = 1;}
