...
class WP_Posts_List_Table extends WP_List_Table {
	...
	function get_views() {
		...
		// print_r(get_post_stati( ));

		// Subtract post types that are not included in the admin all list.
		//get_post_stati( array('show_in_admin_all_list' => false) )
		foreach ( get_post_stati( ) as $state )
			$total_posts -= $num_posts->$state;

		$class = empty( $class ) && empty( $_REQUEST['post_status'] ) && empty( $_REQUEST['show_sticky'] ) ? ' class="current"' : '';
		$status_links['all'] = 
			"<a href='edit.php?post_type=$post_type{$allposts}'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 
			'All <span class="count">(%s)</span>', $total_posts, 'posts' ), number_format_i18n( $total_posts ) ) . '</a>';
