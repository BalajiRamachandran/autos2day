...
case 'logout' :
	...
	$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : esc_url( home_url( '/' ) );//'wp-login.php?loggedout=true';
...
<p id="nav">
<a href="<?php echo esc_url( /*wp_login_url()*/ site_url('login') ); ?>"><?php _e('Log in') ?></a>
...
case 'resetpass' :
case 'rp' :
	$location = "recover-password";//wp-login.php
	$user = check_password_reset_key($_GET['key'], $_GET['login']);
	if ( is_wp_error($user) ) {
		wp_redirect( site_url($location . '?action=lostpassword&error=invalidkey') );
		exit;
	}
	...
	if ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ) {
		...
		login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( site_url( 'login' ) ) . '">' . __( 'Log in' ) . '</a></p>' );
		login_footer();
		exit;
	}
...
default:
	...
	if ( isset( $_REQUEST['redirect_to'] ) ) {
		...
		$redirect_to = get_option('siteurl');//admin_url();
	}
