		...
		<ul class="tabs">
			...
			for($i=0,$a=1;$i<$num;$i++,$a++){
				...
				if($thumb_yes == TRUE) { 						
					...
					?>
					<li>
						<a class="thumbTab imgTab" href="<?php echo $data[$i]['guid']; ?>" rel="zoom-id:zoom1" rev="<?php echo $imgURL[$i];?>">
							<img src="<?php echo $t_imgURL[$i];?>" alt="<?php echo $data[$i]['post_title']; ?>"/>
						</a>
					</li>
				<?php } else { ?>
					<li>
						<a class="thumbTab numTab" href="<?php echo $data[$i]['guid']; ?>" rel="zoom-id:zoom1" rev="<?php echo $imgURL[$i]; ?>">
							<?php echo $a; ?>
						</a>
					</li>
				<?php } 
		...
		<div class="contentWrap mediaPanes">
			...
			if($OPTION['wps_imagesTab_enable']) { ?>
				<div class="theProdMedia innerProdMedia clearfix">
					...
					if($num != 0){ ?>
						<a href="<?php echo $data[0]['guid']; ?>" class="MagicZoom MagicThumb" id="zoom1" rel="show-title: false; zoom-fade: true; zoom-position: inner; selectors-change:mouseover; selectors-effect:fade;"><img src="<?php echo $imgURL[0];?>" alt="<?php echo $data[0]['post_title']; ?>"/></a>
					<?php } else { ?>
						<a href="<?php echo $img_src; ?>" class="MagicZoom MagicThumb" id="zoom1" rel="show-title: false; zoom-fade: true; zoom-position: inner; selectors-change:mouseover; selectors-effect:fade;"><img src="<?php echo $imgURL;?>" alt="<?php echo wp_specialchars( get_the_title($post->ID), 1 ); ?>"/></a>
					<?php } ?>
				</div>
			<?php } 
...
if(empty($videoMatches[1][0])) { 
	...
	<div class="<?php echo $the_div_class;?>">
		<div class="contentWrap">
			<?php if($num != 0){ ?>
				<a href="<?php echo $data[0]['guid']; ?>" class="MagicZoom" id="zoom1" rel="zoom-position: top; zoom-distance: 200px; selectors-change:mouseover; zoom-width:400px; zoom-height:400px;"><img src="<?php echo $imgURL[0];?>" alt="<?php echo $data[0]['post_title']; ?>"/></a>
			<?php } else { ?>
				<a href="<?php echo $img_src; ?>" class="MagicZoom MagicThumb" id="zoom1" rel="show-title: false; zoom-fade: true; zoom-position: inner; selectors-change:mouseover; selectors-effect:fade;zoom-width:400px; zoom-height:400px;"><img src="<?php echo $imgURL;?>" alt="<?php echo wp_specialchars( get_the_title($post->ID), 1 ); ?>"/></a>
			<?php } ?>
		</div>
	</div><!-- c_box  -->
<?php } ?>