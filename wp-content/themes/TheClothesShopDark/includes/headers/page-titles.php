	...
	if ($blog_Name != 'Select a Category') {
		if (is_category()) {
			...
			if((int)$categ_object->category_parent > 0 ) {
				...
				else {include( STYLESHEETPATH . "/includes/headers/sub-cat-navi.php" );}
			} else {
			...
			if ((cat_is_ancestor_of( $blog_ID, (int)$categ_object[0]->term_id ))|| ($categ_object[0]->term_id ==$blog_ID)) {} else {include( STYLESHEETPATH . "/includes/headers/sub-cat-navi.php" );}
		}
	} else {include( STYLESHEETPATH . "/includes/headers/sub-cat-navi.php" );}
