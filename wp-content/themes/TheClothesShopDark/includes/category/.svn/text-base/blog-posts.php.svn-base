<div <?php post_class('blogPost clearfix'); ?> id="post-<?php the_ID(); ?>">
	...
	<div class="entry">
		...
		<div class="clearfix teaser">
			...
			include (STYLESHEETPATH . '/includes/category/blog-post-teaser.php');?>
		</div><!-- teaser -->
		
		<?php if($OPTION['wps_blog_cat_meta_enable']){ ?>
			<p class="meta">
				<?php the_tags( '<span>' . __('Tagged as: ', 'wpShop' ), ',' , '</span>' ); ?> | <span><?php _e( 'Posted in: ', 'wpShop' ); ?><?php echo get_the_category_list(', '); ?></span>
			</p>
		<?php } ?>
