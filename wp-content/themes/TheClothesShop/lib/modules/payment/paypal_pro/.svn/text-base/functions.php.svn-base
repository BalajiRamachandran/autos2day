...
//SN
function cc_start_date_ddown($option='year'){
	...
	if($option == 'year'){
			...
			for($i=0,$y=$year; $i<10; $y--,$i++){
				$selected = ($_SESSION['cc_starts_year'] == $y ? "selected='selected'" : NULL);
				$output .= "<option value='$y' $selected >$y</option>" . "\n";
			}
	}
return $output;
...
function createPProString(){
	...
	$_SESSION['cc_starts_month'] 	= $_POST['cc_starts_month'];
	$_SESSION['cc_starts_year'] 	= $_POST['cc_starts_year'];	
...
function paypal_pro_response(){
	...
	//change.9.12
	$get_t	= trim($_GET['t']);
	$sql	= "SELECT cid FROM $table WHERE who = '$_SESSION[cust_id]' AND transid = '$get_t'";
	//\change.9.12
