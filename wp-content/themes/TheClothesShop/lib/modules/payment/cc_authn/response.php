...
$AUTHN_DATA = authn_response();
	...
	if($AUTHN_DATA['status'] == 1)
	{
			...
			<p>".__('You may also Track your Order using the Tracking Number you are given in your Confirmation Email. Simply login to your Account and enter that number in the field under \'Track Your Order\' in the sidebar.','wpShop').						
			"</p>
			<h4>".__('Print your Order?','wpShop')."</h4>
			<p><a href='{$homeurl}/index.php?display_invoice=1&invoice={$AUTHN_DATA[pdf_bill]}' 
			target='_blank'>$bill_format_label</a></p>";									
