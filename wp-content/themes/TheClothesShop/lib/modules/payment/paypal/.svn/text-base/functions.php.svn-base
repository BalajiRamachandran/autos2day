...
function pdt_response(){
	...
	if (!$fp) {
		...
		if (strcmp ($lines[0], "SUCCESS") == 0) {
			...
			if($paypal_email == $keyarray[business]){
			}
			else {
				$ERROR = 1;
				$PDT_DATA['error_code'] 	= 'PP02';
				$PDT_DATA['error_message']	= __('Error PP02: Receiver-email and Paypal-email are not identical','wpShop')."<br/>";
			}
			...
			if(($ERROR == 0) && ($COMPLETED == 1)){
					...
					// we create a pdf for bill + save name of pdf in PDT-DATA
...
function ipn_response(){
	...
	if(md5(LOGGED_IN_KEY.'-'.NONCE_KEY) != $_GET['pst']){	
		mail($OPTION['wps_shop_email'],'IPN error',__('Please check if your IPN value in PayPal is correctly entered.','wpShop'));
		exit();
	}	 
	...
	$header .= "Content-Length: " . strlen($req) . "\r\n\r\n";
	$fp 	= fsockopen ('ssl://www.paypal.com', 443, $errno, $errstr, 30);	
	//in seldom cases fsockopen can't handle SSL b'c host hasn't activated it, then it will fall back to port 80
	if($fp === FALSE){	
		$fp 	= fsockopen ('www.paypal.com',80,$errno,$errstr,30);	 
	}
	...
	if(!$fp){
		...
		while (!feof($fp))
		{
			...
			if(strcmp($res,"VERIFIED") == 0){
				...
				$IPN_DATA['who'] 			= NWS_decode($keyarray['custom']);	
				...
				if(($ERROR == 0) && ($PENDING == 1)){
					...
					//we create a pdf for bill + save name of pdf in IPN-DATA
						// in case of other languages than english,german,french,italian we produce a html instead
						if(pdf_usable_language()){
							$INVOICE->make_pdf($order);			//change.9.10
							$IPN_DATA[pdf_bill] = $OPTION['wps_invoice_prefix'].'_' . $order[tracking_id] . '.pdf';
