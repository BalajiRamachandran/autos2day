<?php
	class Statistics {
		...
		function graph_monthly_sales($row_height,$txt_color){
			...
			$qStr 	= "SELECT * FROM $table WHERE (order_time BETWEEN $start AND $end) AND level IN ('4','5','6','7')";
			...
			if($num > 0)
			{
				...
				foreach($data as $k => $v){
					...
					color: $txt_color;'></td><td style='color: $txt_color;'>".format_price($mdata[2])." $currency</td>
					</tr></table>
					";
				}
			}
