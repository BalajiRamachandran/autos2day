...
$mainPages = get_pages('child_of='.$currentPage->ID.'&parent='.$currentPage->ID.'&sort_column=menu_order');
foreach($mainPages as $post){$childPages = get_pages('child_of='.$post->ID.'&parent='.$post->ID.'&sort_column=menu_order');}
$counter = 3;
$a = 1;
if (!empty($mainPages)) {
	if (empty($childPages)) {
	...
	<div class="<?php echo $the_div_class; ?>">
		...
		foreach($mainPages as $post){
			...
			$childPages = get_pages('child_of='.$post->ID.'&parent='.$post->ID.'&sort_column=menu_order');
	...
	<?php 
	if (!empty($childPages)) {
		if ( is_sidebar_active('main_customer_service_widget_area') ) : dynamic_sidebar('main_customer_service_widget_area'); endif; 
	}
