		...
		<?php  if($num > 1){ ?>
			<div class="contentWrap mediaPanes mediaPanes_alt">
				<?php for($i=0;$i<$num;$i++){ 
					if($OPTION['wps_caption_enable']) {$title = $data[$i]['post_title'];}else {$title = '';}
				?>
					<a class="fancybox theProdMedia theProdMedia_alt" title="<?php echo $title; ?>" href="<?php echo $data[$i]['guid']; ?>" rel="imgGroup"><img src="<?php echo $imgURL[$i];?>" alt="<?php echo $data[$i]['post_title']; ?>"/></a>
				<?php } ?>
			</div>
		<?php } else { 
			if($num != 0){ ?>
