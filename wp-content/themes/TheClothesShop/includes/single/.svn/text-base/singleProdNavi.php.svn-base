...
if (!empty($previousPost) || !empty($nextPost)) { ?>	
	...
	<div class="prodNav noprint">
		...
		<span id="previousHover" class="adjacentProd previousProd">
			<?php 
				NWS_previous_post_link('%link', '<img src="'.get_bloginfo('stylesheet_directory').'/images/arrow_left_16.png" alt="'.$OPTION['wps_prevProdLinkText'].'" /> '.$OPTION['wps_prevProdLinkText'].'', TRUE);
	...
	| 
		...
		<span id="nextHover" class="adjacentProd nextProd">
			<?php 
				NWS_next_post_link('%link', ''.$OPTION['wps_nextProdLinkText'].' <img src="'.get_bloginfo('stylesheet_directory').'/images/arrow_right_16.png" alt="'.$OPTION['wps_nextProdLinkText'].'" />', TRUE);
