...
the_post(); ?>
	...
	<div id="singleMainContent" class="clearfix">
		...
		if($_GET['added'] == 'OK'){
			...
			if ( strlen($_POST['voucher_email']) > 0 ) {
				...
				if ( $num > 0 ) {
					...
					$qStr 			="INSERT INTO $table (email,dealership_id,user_id,vin,voucher,post_id,date_sent) VALUES ('$email_address','$dealership_id','$author','$post_vin', '$voucher_code', '$post_id', CURDATE())";
		...
		//\change.9.9		
		?>			
		<h1 class="entry-title single-page"><?php the_title(); ?><?php include(TEMPLATEPATH . "/lib/pages/single_product_status.php");?></h1>			
		...
		<div class="prodDetails singleDetailsPage">
			...
			<?php 
				if (!empty($videoMatches[1][0])) { 
					...
					include (TEMPLATEPATH . '/lib/pages/single_deal_details.php');
...
<!-- 
		...
		<?php if($OPTION['wps_prodNav_enable']) { ?>
			<div class="linksBottom clearfix noprint">
				<?php include (TEMPLATEPATH . '/includes/single/singleProdNavi.php');?>
			</div>
		<?php } ?>
	...
	<div id="sizesInfoOverlay" class="overlay" style="width: auto; height: 200px;">
			<!--cforms name="Get_voucher"-->
			<?php 
			// insert_cform('2');
			?>
			<form action="#" method="post" name="get_voucher">
				<fieldset>
					<label for="voucher_email">Enter your email address</label>
					<input type="text" value="" tabindex="35" name="voucher_email" id="voucher_email" required/>
